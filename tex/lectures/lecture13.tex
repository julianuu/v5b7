...
\begin{lem}[Cotlar-Knapp-Stein]
	Assume for $\|T_kT_j^*\|_{L^2→L^2}$ that $\|T_k^*T_j\|\leq a(k-j)^2$. Then, for eveny $r$,
	\[\|\sum_{k=0}^rT_k\|\leq A.\]
...
The estimates 1 and 2 impl that the hypotheses of CKS ar satisfied with $a(k)=c_n2^{-|k|N}$ ($∀N\therefore$ can choose $N=1$).
\[\therefore\|∂_x^α\sum_{k=0}^rA_k(f)\|_{L^2}\leq A\|f\|_{L^2}\]
wher $A=\sum_{k∈ℤ}$, provided $|α|\leq\f{d-1}2$.
\begin{enumerate}
